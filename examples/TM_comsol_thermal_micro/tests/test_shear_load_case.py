...
def test_s0070_shear_schedule_extends_s0050_conservatively():
    s0050 = [
        float(line.strip())
        for line in (ROOT / "load_schedules" / "load_schedule_S0050_shear.csv").read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
    s0070_path = ROOT / "load_schedules" / "load_schedule_S0070_shear.csv"

    assert s0070_path.exists()
    s0070 = [float(line.strip()) for line in s0070_path.read_text(encoding="utf-8").splitlines() if line.strip()]

    assert s0070[: len(s0050)] == s0050
    assert 41 <= len(s0070) <= 51
    assert s0070[-1] == 1.0e-4
    assert max(b - a for a, b in zip(s0070, s0070[1:]) if a >= 8.0e-5) <= 2.0e-6 + 1.0e-12


def test_s0090_shear_schedule_extends_s0070_conservatively():
    s0070 = [
        float(line.strip())
        for line in (ROOT / "load_schedules" / "load_schedule_S0070_shear.csv").read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
    s0090_path = ROOT / "load_schedules" / "load_schedule_S0090_shear.csv"

    assert s0090_path.exists()
    s0090 = [float(line.strip()) for line in s0090_path.read_text(encoding="utf-8").splitlines() if line.strip()]

    assert s0090[: len(s0070)] == s0070
    assert 51 <= len(s0090) <= 61
    assert s0090[-1] == 1.2e-4
    assert max(b - a for a, b in zip(s0090, s0090[1:]) if a >= 1.0e-4) <= 2.0e-6 + 1.0e-12
