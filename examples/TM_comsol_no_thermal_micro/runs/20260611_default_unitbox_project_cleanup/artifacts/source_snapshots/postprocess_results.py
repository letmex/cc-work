"""Generate stress-strain/reaction tables and figures for TM_comsol_no_thermal_micro.
...
available, this module still writes a stress-strain table with
...
# Deprecated compatibility constants. New code should use the functional names above.
CORRECTED_CURVE_NAME = STRESS_STRAIN_TABLE_NAME
CORRECTED_REACTION_NAME = REACTION_TABLE_NAME
AVAILABILITY_NAME = REACTION_AVAILABILITY_NAME
...
def build_corrected_stress_strain_curve(
    ...
    if "reaction_N_energy_exact" not in data.columns:
        raise RuntimeError("Reaction table lacks reaction_N_energy_exact")
...
def build_stress_strain_curve(
    ...
    return build_corrected_stress_strain_curve(
        reactions,
...
def write_unavailable_stress_strain_curve(
    ...
    return write_unavailable_corrected_curve(
        result_dir,
        out_path,
        reason,
        reference_length_mm=reference_length_mm,
...
def generate_clean_figures_for_corrected_curve(
    ...
    return generate_figures_for_stress_strain_curve(
        result_dir,
        corrected_curve,
...
def run_results_postprocess(
    ...
    selected_seed = seed if seed is not None else corrected_seed if corrected_seed is not None else settings_seed
    ckpts = checkpoint_paths(model_dir)
    if not ckpts:
        write_unavailable_stress_strain_curve(result_dir, curve_path, "no_step_checkpoints", seed=settings_seed)
        _write_availability(
            out_dir,
            {
    ...
    try:
        ...
        write_unavailable_stress_strain_curve(result_dir, curve_path, reason, seed=settings_seed)
        _write_availability(
            out_dir,
            {
                "status": reason,
...
def run_corrected_reaction_postprocess(*args, **kwargs) -> dict[str, object]:
    return run_results_postprocess(*args, **kwargs)
