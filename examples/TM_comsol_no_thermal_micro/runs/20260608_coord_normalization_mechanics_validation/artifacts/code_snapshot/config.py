...
def _parse_args():
    ...
    parser.add_argument(
        "--top-u-mode",
        choices=["fixed", "free"],
        default=os.getenv("TM_COMSOL_MICRO_TOP_U_MODE", "fixed"),
        help="Experimental displacement ansatz switch. Default fixed preserves old top u=0 behavior.",
    )
    parser.add_argument(
        "--coord-normalization",
        choices=["none", "unit_box"],
        default=os.getenv("TM_COMSOL_MICRO_COORD_NORMALIZATION", "none"),
        help=(
            "Coordinate input mapping for the NN only. Default none preserves physical-mm "
            "network inputs; unit_box maps x,y to [-1,1] before self.net while all "
            "T3 gradients, boundary values, and energies remain in physical units."
        ),
    )
    parser.add_argument(
        "--phase-proximal",
    ...
    parser.add_argument(
        ...
        default=False,
        type=_str_to_bool,
        help="Save composite per-loading-step checkpoints. Default false.",
    )
    parser.add_argument(
