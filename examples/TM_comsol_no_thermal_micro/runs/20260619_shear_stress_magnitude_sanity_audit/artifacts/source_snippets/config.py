...
def _parse_args():
    ...
    parser.add_argument(
        ...
        default=os.getenv("TM_COMSOL_MICRO_LOAD_CASE", "tension"),
        help="Mechanical load case. Default tension is the verified normal route.",
    )
    parser.add_argument(
        "--GcII",
        type=float,
...
model_name = (
    ...
    f"_PFFmodel_AT2"
    f"_l0_{l0_label}"
    f"_comsolMicroNoThermal_TM_verified{load_case_label}"
    f"{tm_eps_r_label}"
    f"{top_u_label}"
    f"{coord_norm_label}"
    f"{optimizer_label}"
...
with open(model_path / Path("model_settings.txt"), "w", encoding="utf-8") as file:
    ...
    file.write(f"\nload_case: {training_dict['load_case']}")
    file.write(f"\nload_parameter_name: {'Delta_s' if training_dict['load_case'] == 'shear' else 'Delta'}")
