...
def _infer_top_u_mode(run_dir, case_name):
    text = f"{run_dir.name} {case_name}".lower()
    return "free" if "topufree" in text or "top-u-free" in text else "fixed"


def _boundary_displacement_stats(npz, displacement, top_u_mode):
    x = _safe_array(npz, "x")
    y = _safe_array(npz, "y")
    u = _safe_array(npz, "u")
    v = _safe_array(npz, "v")
    if x is None or y is None or u is None or v is None:
        return {"top_u_mode": top_u_mode}
    ...
    def masked_stats(values, mask, prefix):
        ...
        return {
            ...
            f"{prefix}_abs_max": float(np.max(np.abs(subset))),
        }

    stats = {"top_u_mode": top_u_mode}
    stats.update(masked_stats(u, top_mask, "top_u"))
    stats.update(masked_stats(v, top_mask, "top_v"))
    stats.update(masked_stats(u, bottom_mask, "bottom_u"))
    stats.update(masked_stats(v, bottom_mask, "bottom_v"))
    if np.any(top_mask):
        stats["top_v_error_max"] = float(np.max(np.abs(v[top_mask] - float(displacement))))
    else:
...
def analyze(run_dir, out_csv, events_csv, summary_md, case_name):
    ...
    rows = []
    inferred_top_u_mode = _infer_top_u_mode(run_dir, case_name)
    ...
    for path in npz_paths:
        ...
        with np.load(path) as npz:
            ...
            boundary_stats = _boundary_displacement_stats(npz, row["Delta"], inferred_top_u_mode)
            ...
            row["He_current_ratio_denominator_valid"] = bool(
                np.isfinite(row["notch_tip_He_current_max"])
                and row["notch_tip_He_current_max"] > RATIO_VALID_NOTCH_HE_THRESHOLD
            )
            row["mechanics_drive_ratio_denominator_valid"] = bool(
                np.isfinite(row["notch_tip_mechanics_drive_max"])
                and row["notch_tip_mechanics_drive_max"] > RATIO_VALID_NOTCH_HE_THRESHOLD
            )
            row["bulk_He_current_p95_over_notch_tip_He_current_max_valid"] = (
                row["bulk_He_current_p95_over_notch_tip_He_current_max"]
                if row["He_current_ratio_denominator_valid"]
                else np.nan
            )
            row["bottom_right_He_current_max_over_notch_tip_He_current_max_valid"] = (
                row["bottom_right_He_current_max_over_notch_tip_He_current_max"]
                if row["He_current_ratio_denominator_valid"]
                else np.nan
            )
            row["bulk_mechanics_drive_p95_over_notch_tip_mechanics_drive_max_valid"] = (
                row["bulk_mechanics_drive_p95_over_notch_tip_mechanics_drive_max"]
                if row["mechanics_drive_ratio_denominator_valid"]
                else np.nan
            )
            row["bottom_right_mechanics_drive_max_over_notch_tip_mechanics_drive_max_valid"] = (
                row["bottom_right_mechanics_drive_max_over_notch_tip_mechanics_drive_max"]
                if row["mechanics_drive_ratio_denominator_valid"]
                else np.nan
            )
            ...
            for key in (
                ...
                row[key] = diag_row.get(key) if key == "top_u_mode" else _safe_float(diag_row.get(key))
            for key, value in boundary_stats.items():
                if (
                    key not in row
                    or row[key] is None
                    or row[key] == ""
                    or (key != "top_u_mode" and not np.isfinite(row[key]))
                ):
                    row[key] = value
            rows.append(row)
    ...
    preferred = [
        ...
        "He_current_ratio_denominator_valid",
        "mechanics_drive_ratio_denominator_valid",
        "bulk_He_current_p95_over_notch_tip_He_current_max_valid",
        "bottom_right_He_current_max_over_notch_tip_He_current_max_valid",
        "bulk_mechanics_drive_p95_over_notch_tip_mechanics_drive_max_valid",
        "bottom_right_mechanics_drive_max_over_notch_tip_mechanics_drive_max_valid",
    ...
    event_defs = [
        ...
        ("first_bulk_He_ratio_gt_0p25", "bulk_He_current_p95_over_notch_tip_He_current_max_valid", 0.25),
        ("first_bulk_He_ratio_gt_0p5", "bulk_He_current_p95_over_notch_tip_He_current_max_valid", 0.5),
        ("first_bulk_drive_ratio_gt_0p25", "bulk_mechanics_drive_p95_over_notch_tip_mechanics_drive_max_valid", 0.25),
        ("first_bulk_drive_ratio_gt_0p5", "bulk_mechanics_drive_p95_over_notch_tip_mechanics_drive_max_valid", 0.5),
        ("first_bottom_He_ratio_gt_0p5", "bottom_right_He_current_max_over_notch_tip_He_current_max_valid", 0.5),
        ("first_bottom_drive_ratio_gt_0p5", "bottom_right_mechanics_drive_max_over_notch_tip_mechanics_drive_max_valid", 0.5),
        ("first_notch_He_gt_1e-8", "notch_tip_He_current_max", 1.0e-8),
        ("first_notch_He_gt_1e-6", "notch_tip_He_current_max", 1.0e-6),
        ("first_bulk_He_p95_gt_1e-8", "bulk_He_current_p95", 1.0e-8),
        ("first_bulk_He_p95_gt_1e-6", "bulk_He_current_p95", 1.0e-6),
    ]
    events = []
    ...
    ratio_hit = None
    for row in rows:
        if row.get("He_current_ratio_denominator_valid"):
            ratio_hit = row
            break
    events.append(
        {
            "case": case_name,
            "event": "first_step_where_ratio_valid",
            "metric": "notch_tip_He_current_max",
            "threshold": RATIO_VALID_NOTCH_HE_THRESHOLD,
            "step": ratio_hit["step"] if ratio_hit else "",
            "Delta": ratio_hit["Delta"] if ratio_hit else "",
            "value": ratio_hit["notch_tip_He_current_max"] if ratio_hit else "",
        }
    )
    ...
    with summary_md.open("w", encoding="utf-8") as handle:
        ...
        handle.write(
            "Ratio validity threshold: `notch_tip_He_current_max > "
            f"{RATIO_VALID_NOTCH_HE_THRESHOLD}`.\n\n"
        )
