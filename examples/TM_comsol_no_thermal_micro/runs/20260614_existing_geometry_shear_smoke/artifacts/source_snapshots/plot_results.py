...
def infer_short_run_label(path):
    ...
    load = None
    for candidate in ("S0020", "D0020", "D0040"):
        if candidate in text:
            load = candidate
            break
    if seed is not None and load is not None and "shear" in text.lower():
        return f"seed{seed}_shear"
...
def _normalise_stress_strain_rows(rows):
    ...
    for row in rows:
        ...
        if item.get("load_case") == "shear" or "engineering_shear_strain" in item:
            if "strain" not in item and "engineering_shear_strain" in item:
                item["strain"] = item["engineering_shear_strain"]
...
def unavailable_reaction_rows(paths, specimen_height=1.0, load_case="tension"):
    rows = []
    for path in paths:
        data = np.load(path)
        ...
        if load_case == "shear":
            row["Delta_s"] = delta
            row["engineering_shear_strain"] = strain
            row["nominal_shear_stress_energy_MPa"] = np.nan
        else:
            row["nominal_stress_energy_MPa"] = np.nan
        rows.append(row)
    return rows
...
def plot_stress_strain(rows, out_path, dpi=300):
    is_shear = any(row.get("load_case") == "shear" or "engineering_shear_strain" in row for row in rows)
    if is_shear:
        strain = _row_series(rows, "engineering_shear_strain", "strain")
        stress_primary = _row_series(rows, "nominal_shear_stress_energy_MPa")
        stress_virtual = _row_series(rows, "nominal_shear_stress_virtual_work_MPa")
        xlabel = "Engineering shear strain"
        ylabel = "Nominal shear stress [MPa]"
    else:
...
def plot_reaction_strain(rows, out_path, dpi=300):
    is_shear = any(row.get("load_case") == "shear" or "engineering_shear_strain" in row for row in rows)
    strain = _row_series(rows, "engineering_shear_strain", "strain") if is_shear else _row_series(rows, "nominal_strain", "strain")
    ...
    ax.set_xlabel("Engineering shear strain" if is_shear else "Engineering strain")
...
def make_result_figures(
    ...
    for spec in FIELD_SPECS:
        if spec["key"] == "disp" or spec["key"] in final_data:
            ...
            for alias in spec.get("aliases", ()):
                alias_path = out_dir / f"final_{alias}_{label}.png"
                plot_field(final_data, spec, alias_path, run_label=run_label or result_dir.name, dpi=dpi)
    ...
    load_case = "shear" if table_rows is not None and any(row.get("load_case") == "shear" for row in table_rows) else "tension"
    rows = (
        table_rows
        if table_rows is not None
        else unavailable_reaction_rows(paths, specimen_height=specimen_height, load_case=load_case)
    )
        ...
        metadata_path.write_text(
            "\n".join(
                [
                    ...
                    "primary_metric=nominal_shear_stress_energy_MPa" if load_case == "shear" else "primary_metric=nominal_stress_energy_MPa",
                ]
            )
            + "\n",
            encoding="utf-8",
