...
def _has_cols(data, *keys):
    return data is not None and data.dtype.names is not None and all(key in data.dtype.names for key in keys)


def _curve_values(data, x_key, y_key):
    return np.asarray(data[x_key], dtype=float), np.asarray(data[y_key], dtype=float)


def _resolve_stress_strain_curve_source(results_path, curve_file):
    """Pick the primary stress-strain curve source.

    Corrected checkpoint reactions are the primary source when available. The
    legacy top-boundary stress integral remains a diagnostic overlay only.
    """
    results_path = Path(results_path)
    curve_file = Path(curve_file)
    candidates = [
        results_path / Path("curves/corrected_stress_strain_by_step.csv"),
        results_path / Path("curves/reaction_displacement_macro_stress_strain_corrected.csv"),
        curve_file,
    ]
    primary = None
    for candidate in candidates:
        data = _read_named_csv(candidate)
        if _has_cols(data, "nominal_strain", "nominal_stress_energy_exact_MPa"):
            x, y = _curve_values(data, "nominal_strain", "nominal_stress_energy_exact_MPa")
            primary = {
                "available": True,
                "path": str(candidate),
                "x": x,
                "y": y,
                "x_label": "Nominal strain",
                "y_label": "Nominal stress from energy-conjugate reaction (MPa)",
                "label": "primary energy-conjugate",
                "metric": "nominal_stress_energy_exact_MPa",
            }
            if _has_cols(data, "Delta", "reaction_N_energy_exact"):
                primary["disp_x"] = np.asarray(data["Delta"], dtype=float)
                primary["reaction_y"] = np.asarray(data["reaction_N_energy_exact"], dtype=float)
            break
        if _has_cols(data, "macro_strain", "macro_stress_energy_exact"):
            x, y = _curve_values(data, "macro_strain", "macro_stress_energy_exact")
            primary = {
                "available": True,
                "path": str(candidate),
                "x": x,
                "y": y,
                "x_label": "Macro strain",
                "y_label": "Macro stress from energy-conjugate reaction",
                "label": "primary energy-conjugate",
                "metric": "macro_stress_energy_exact",
            }
            break

    legacy_data = _read_named_csv(curve_file)
    legacy = None
    if _has_cols(legacy_data, "macro_strain", "macro_stress"):
        x, y = _curve_values(legacy_data, "macro_strain", "macro_stress")
        legacy = {
            "available": True,
            "path": str(curve_file),
            "x": x,
            "y": y,
            "x_label": "Macro strain",
            "y_label": "Legacy macro stress",
            "label": "legacy top-boundary sigma diagnostic",
            "metric": "macro_stress",
        }
        if _has_cols(legacy_data, "uy_top", "reaction_force"):
            legacy["disp_x"] = np.asarray(legacy_data["uy_top"], dtype=float)
            legacy["reaction_y"] = np.asarray(legacy_data["reaction_force"], dtype=float)

    return {
        "primary": primary,
        "legacy": legacy,
        "status": "energy_conjugate_primary" if primary is not None else "reaction_metric_unavailable",
    }


def _write_stress_strain_source_report(fig_path, curve_source):
    report = fig_path / Path("stress_strain_curve_source.txt")
    primary = curve_source.get("primary")
    legacy = curve_source.get("legacy")
    with open(report, "w", encoding="utf-8") as handle:
        handle.write(f"status={curve_source.get('status')}\n")
        if primary is not None:
            handle.write(f"primary_metric={primary.get('metric')}\n")
            handle.write(f"primary_path={primary.get('path')}\n")
        else:
            handle.write("primary_metric=reaction_metric_unavailable\n")
            handle.write("primary_reason=no corrected checkpoint energy reaction curve found\n")
        if legacy is not None:
            handle.write(f"legacy_metric={legacy.get('metric')}\n")
            handle.write(f"legacy_path={legacy.get('path')}\n")
            handle.write("legacy_status=diagnostic_only\n")
...
def postprocess_tm(results_path, inp, T_conn, step_idx=-1, dpi=300, bc_dict=None):
    ...
    if curve_file.exists() or (results_path / Path("curves/corrected_stress_strain_by_step.csv")).exists():
        curve_source = _resolve_stress_strain_curve_source(results_path, curve_file)
        _write_stress_strain_source_report(fig_path, curve_source)
        primary_curve = curve_source.get("primary")
        legacy_curve = curve_source.get("legacy")

        fig1, ax1 = plt.subplots(figsize=(4.8, 3.3))
        if primary_curve is not None and "disp_x" in primary_curve and "reaction_y" in primary_curve:
            ax1.plot(primary_curve["disp_x"], primary_curve["reaction_y"], "-k", label="primary energy-conjugate")
            ax1.set_title("Reaction-Displacement")
        elif legacy_curve is not None and "disp_x" in legacy_curve and "reaction_y" in legacy_curve:
            ax1.plot(legacy_curve["disp_x"], legacy_curve["reaction_y"], "--", color="0.35", label="legacy top sigma diagnostic")
            ax1.set_title("Legacy Reaction-Displacement Diagnostic")
        else:
            ax1.text(0.5, 0.5, "Reaction curve unavailable", ha="center", va="center", transform=ax1.transAxes)
            ax1.set_title("Reaction-Displacement Unavailable")
        ax1.set_xlabel("Top displacement / Delta")
        ...
        if legacy_curve is not None:
            fig_legacy, ax_legacy = plt.subplots(figsize=(4.8, 3.3))
            ax_legacy.plot(legacy_curve["x"], legacy_curve["y"], "--", color="0.35")
            ax_legacy.set_xlabel(legacy_curve["x_label"])
            ax_legacy.set_ylabel(legacy_curve["y_label"])
            ax_legacy.set_title("Legacy Stress-Strain Diagnostic")
            ax_legacy.grid(alpha=0.25)
            plt.savefig(fig_path / Path("macro_stress_strain_legacy.png"), dpi=dpi, bbox_inches="tight")
            plt.close(fig_legacy)

        fig2, ax2 = plt.subplots(figsize=(4.8, 3.3))
        if primary_curve is not None:
            ax2.plot(primary_curve["x"], primary_curve["y"], "-k", label=primary_curve["label"])
            if legacy_curve is not None:
                ax2.plot(legacy_curve["x"], legacy_curve["y"], "--", color="0.55", label=legacy_curve["label"])
            ax2.set_xlabel(primary_curve["x_label"])
            ax2.set_ylabel(primary_curve["y_label"])
            ax2.set_title("Corrected Macro Stress-Strain")
        else:
            ax2.text(
                0.5,
                0.58,
                "Primary energy-conjugate stress-strain unavailable",
                ha="center",
                va="center",
            ...
            ax2.text(
                0.5,
                0.42,
                "Legacy top sigma was written to macro_stress_strain_legacy.png",
                ha="center",
                va="center",
                transform=ax2.transAxes,
                fontsize=8,
            ...
            ax2.set_ylabel("Primary macro stress")
            ax2.set_title("Macro Stress-Strain Unavailable")
        ax2.grid(alpha=0.25)
        if len(ax2.lines) > 0:
