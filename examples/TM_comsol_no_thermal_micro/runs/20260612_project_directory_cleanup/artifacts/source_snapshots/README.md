...
The example root is source-only. Generated training outputs, checkpoints,
figures, curves, logs, and temporary diagnostics are written under `outputs/`.
Audit packages and handoff packages belong under `runs/`.
...
inside a named `runs/` audit package. Do not commit generated CSV/NPZ/checkpoint
or run folders unless they are intentionally reduced into a small audit table.
