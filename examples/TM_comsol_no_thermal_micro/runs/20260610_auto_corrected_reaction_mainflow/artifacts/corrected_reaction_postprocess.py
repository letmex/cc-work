...
def run_corrected_reaction_postprocess(
    ...
    if not ckpts:
        ...
        return {"status": "no_step_checkpoints", "corrected_curve": curve_path, "exact_reaction_computable": False}
    try:
        ...
        return {"status": "energy_exact_primary", "corrected_curve": curve_path, "exact_reaction_computable": True}
        ...
        return {"status": reason, "corrected_curve": curve_path, "exact_reaction_computable": False}


def main() -> None:
    import argparse

    parser = argparse.ArgumentParser(description="Generate corrected TM stress-strain CSVs from step checkpoints.")
    parser.add_argument("--model-dir", type=Path, required=True)
    parser.add_argument("--result-dir", type=Path, required=True)
    parser.add_argument("--out-dir", type=Path, default=None)
    parser.add_argument("--device", default="cpu")
    args = parser.parse_args()
    result = run_corrected_reaction_postprocess(args.model_dir, args.result_dir, args.out_dir, device=args.device)
    print(result)
