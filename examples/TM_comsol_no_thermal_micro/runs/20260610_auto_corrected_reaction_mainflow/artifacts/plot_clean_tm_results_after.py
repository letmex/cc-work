...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate clean COMSOL micro no-thermal TM cloud plots.")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--result-dir", type=Path, help="Directory containing fields_mixed_tm_step_*.npz files.")
    group.add_argument("--suffix", help="Unique suffix of a result directory under ./results.")
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional corrected curve table. When present, "
            "nominal_stress_energy_exact_MPa is the primary stress-strain source."
        ),
    )
    parser.add_argument(
        "--corrected-seed",
        type=int,
        default=None,
        help="Seed to select when the corrected stress-strain CSV contains multiple seeds.",
    )
...
def find_result_dir(suffix):
    matches = sorted([p for p in (ROOT / "results").iterdir() if p.is_dir() and p.name.endswith(suffix)])
    if len(matches) != 1:
...
def _candidate_corrected_csvs(result_dir, out_dir):
    names = ("corrected_stress_strain_by_step.csv",)
    roots = [
        Path(result_dir),
...
def _normalise_corrected_rows(rows):
    normalised = []
    for row in rows:
        item = dict(row)
...
def load_corrected_stress_strain_rows(path, result_dir, corrected_seed=None):
    ...
    if not rows:
        raise RuntimeError(f"Corrected stress-strain CSV is empty: {path}")
    seeds = sorted({seed for seed in (_as_int(row.get("seed")) for row in rows) if seed is not None})
    selected_seed = corrected_seed if corrected_seed is not None else _infer_seed_from_name(result_dir)
    if selected_seed is not None and seeds:
        rows = [row for row in rows if _as_int(row.get("seed")) == selected_seed]
        if not rows:
            raise RuntimeError(f"No rows for seed {selected_seed} in corrected stress-strain CSV: {path}")
    elif len(seeds) > 1:
        raise RuntimeError(
            "Corrected stress-strain CSV contains multiple seeds; pass --corrected-seed "
            "or use a result directory name containing Seed_<n>."
        )
    return _normalise_corrected_rows(rows), selected_seed
...
def resolve_corrected_stress_strain(result_dir, out_dir, corrected_csv=None, corrected_seed=None):
    if corrected_csv is not None:
        ...
        if not path.exists():
            raise RuntimeError(f"Corrected stress-strain CSV does not exist: {path}")
        rows, seed = load_corrected_stress_strain_rows(path, result_dir, corrected_seed=corrected_seed)
        return rows, path, seed
    for path in _candidate_corrected_csvs(result_dir, out_dir):
        rows, seed = load_corrected_stress_strain_rows(path, result_dir, corrected_seed=corrected_seed)
        return rows, path, seed
    return None, None, corrected_seed
        ...
        rows.append(
            {
                ...
                "stress_strain_metric_status": "corrected_reaction_unavailable",
...
def make_clean_figures(
    ...
    label = _clean_label(run_label or result_dir.name)
    ...
    corrected_rows, corrected_csv, selected_seed = resolve_corrected_stress_strain(
        result_dir,
        out_dir,
        corrected_csv=corrected_stress_strain_csv,
    ...
    if corrected_csv is not None:
        ...
        metadata_path.write_text(
            "\n".join(
                [
                    f"source=corrected_energy_conjugate",
                    f"corrected_csv={_ascii_metadata_path(corrected_csv)}",
...
def main():
    ...
    out_dir = args.out_dir or (ROOT / "results" / "clean_figures" / _clean_label(args.run_label or result_dir.name))
    generated = make_clean_figures(
        result_dir,
        out_dir,
        run_label=args.run_label or result_dir.name,
        specimen_height=args.specimen_height,
        section_area=args.section_area,
        corrected_stress_strain_csv=args.corrected_stress_strain_csv,
